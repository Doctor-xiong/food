        ...
        <c:forEach var="desk" items="${list}" varStatus="d">
            <c:choose>
                <c:when test="${desk.d_position==1}">
                    <a>
                        <div class="table table_busy_${d.count}"><span
                                class="layui-badge">${d.count}号  &nbsp&nbsp已使用</span></div>
                    </a>
                </c:when>
                <c:when test="${desk.d_position==0}">
                    <a>
                        <div class="table table_free_${d.count}"><span class="layui-badge layui-bg-blue">${d.count}号  &nbsp&nbsp待使用</span>
                        </div>
                    </a>
                </c:when>
            </c:choose>
