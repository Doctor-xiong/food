...
public interface OrderitemMapper {
	...
	@Results({
		@Result(column="m_id",property="menu",
				one=@One(select="com.xjt.food.mapper.MenuMapper.queryMenuById"))
	})
	 ...
	 * @return
	 */
	@Insert("insert into orderitem (os_id,oi_id,m_id,oi_num,oi_price) values (#{os_id},#{oi_id},#{m_id},#{oi_num},#{oi_price})")
	 ...
	 * @return
	 */
	@Update("update orderitem set oi_num = (oi_num+1),oi_price = " +
			"(oi_price+(select m_price from menu where menu.m_id = #{m_id}) ) " +
			"where os_id = #{os_id}")
	int updateOrderitemAdd(@Param("os_id") Long os_id, @Param("m_id") Long m_id);
	...
	@Results({
		@Result(column="m_id",property="menu",
				one=@One(select="MenuMapper.queryMenuById"))
	})
