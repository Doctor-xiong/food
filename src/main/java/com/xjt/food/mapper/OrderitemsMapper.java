...
public interface OrderitemsMapper {
	...
	@Select("select * from Orderitems where d_id = #{d_id} and os_position = #{os_position} ")
		@Results({
			@Result(column="os_id",property="oiList",
					many=@Many(select="OrderitemMapper.queryItemByOsid"))
	 ...
	 * 更新用户是否支付的状态
	 */
	@Update("update orderitems set os_position = 0 where os_id = #{os_id}")
	int updateOrderitemsStatus(@Param("os_id") Long id);
}
