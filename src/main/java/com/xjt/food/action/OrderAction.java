...
import com.xjt.food.service.IMenuService;
import com.xjt.food.service.IOrderitemService;
import com.xjt.food.service.IOrderitemsService;
import com.xjt.food.utils.PageUtils;
import com.xjt.food.entity.*;
...
public class OrderAction {
    ...
    public @ResponseBody
    String addMenu(String m_id, HttpSession session) {
        long m_id1 = Long.parseLong(m_id);
        Desk desk = (Desk) session.getAttribute("desk");
        //根据菜品id获取菜品信息
        Menu menu = menuServiceImpl.queryMenuById(m_id1);
        //查询该餐桌是否有状态为0的总订单
        List<Orderitems> osList = orderitemsServiceImpl
                .queryOrderitemsByPosition(0, desk.getD_id());
        //如果没有则 创建一个该餐桌的总订单
        if (osList.size() == 0) {
        ...
        if (osList.size() == 1) {
            ...
            Orderitem oi1 = orderitemServiceImpl
                    .queryOrderitemByMidAndOsid(menu.getM_id(), os1.getOs_id());
    ...
    public @ResponseBody
    String decMenu(String m_id, HttpSession session) {
        long m_id1 = Long.parseLong(m_id);
        Desk desk = (Desk) session.getAttribute("desk");
    ...
    public @ResponseBody
    int delOrderitemsMsg(Model model, HttpSession session, String os_id) {
    ...
    public @ResponseBody
    OredritemsTableModel queryOrderitemsMsgByOsIDorDid(Model model, HttpSession session, String keyWord, String keyType) {
